
export const avatar = {
  en: {
    upload: {
      title: "Upload Avatar",
      description: "Click to upload a new avatar image",
      dragDrop: "Drag and drop an image here",
      or: "or",
      browse: "browse files",
      success: {
        title: "Avatar Updated",
        message: "Your avatar has been successfully updated"
      },
      error: {
        title: "Upload Failed",
        message: "Failed to upload avatar. Please try again.",
        invalidType: "Invalid file type. Please upload an image file.",
        tooLarge: "File is too large. Maximum size is 2MB."
      }
    }
  },
  fr: {
    upload: {
      title: "Télécharger l'avatar",
      description: "Cliquez pour télécharger une nouvelle image d'avatar",
      dragDrop: "Glissez et déposez une image ici",
      or: "ou",
      browse: "parcourir les fichiers",
      success: {
        title: "Avatar mis à jour",
        message: "Votre avatar a été mis à jour avec succès"
      },
      error: {
        title: "Échec du téléchargement",
        message: "Échec du téléchargement de l'avatar. Veuillez réessayer.",
        invalidType: "Type de fichier invalide. Veuillez télécharger un fichier image.",
        tooLarge: "Le fichier est trop volumineux. La taille maximale est de 2 Mo."
      }
    }
  },
  es: {
    upload: {
      title: "Subir avatar",
      description: "Haga clic para subir una nueva imagen de avatar",
      dragDrop: "Arrastre y suelte una imagen aquí",
      or: "o",
      browse: "buscar archivos",
      success: {
        title: "Avatar actualizado",
        message: "Su avatar ha sido actualizado exitosamente"
      },
      error: {
        title: "Error de carga",
        message: "Error al subir el avatar. Por favor, intente de nuevo.",
        invalidType: "Tipo de archivo inválido. Por favor, suba un archivo de imagen.",
        tooLarge: "El archivo es demasiado grande. El tamaño máximo es de 2MB."
      }
    }
  }
};

